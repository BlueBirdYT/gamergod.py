...
async def on_ready():
    ...
    print('--------')
    print('--------')
    print('Started Gamer God') 
    return await client.change_presence(game=discord.Game(name='g!help')) 
...
async def help(ctx):
    if ctx.message.author.bot:
      ...
      embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
      embed.add_field(name = 'Having doubts? Join our server and clear your doubts. Server link:',value ='https://discord.gg/x82HrA9',inline = False)
...
def is_owner(ctx):
     return ctx.message.author.id in ["551320853128806411"]
...
async def setuplog(ctx):
    ...
    if ctx.message.author.server_permissions.administrator == False:
      ...
      everyone_perms = discord.PermissionOverwrite(send_messages=False, read_messages=True)
      everyone = discord.ChannelPermissions(target=server.default_role, overwrite=everyone_perms)
      await client.create_channel(server, '╰☆☆-GamerGod-log-☆☆╮',everyone)
